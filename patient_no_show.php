...
// Fetch no-show appointments with treatment notes, notify_patient status, and review details
try {
    $stmt = $conn->prepare("
        SELECT 
            a.appointment_id, 
    ...
    $stmt->execute();
    $no_show_appointments_result = $stmt->get_result();
    $stmt->close();

} catch (mysqli_sql_exception $e) {
    $error_message = "Error fetching no-show appointments: " . $e->getMessage();
}
...
<head>
    ...
    <title>No-Show Appointments - Dental Clinic Management System</title>
    ...
    <main class="main-content">
        <div class="dashboard-container">
            ...
            <section class="appointments">
                ...
                <?php elseif ($no_show_appointments_result->num_rows > 0): ?>
                    <table>
                        <thead>
                            <tr>
                                <th class="hidden-column">Appointment ID</th> <!-- Hidden column -->
                                <th>Date</th>
                                <th>Time</th>
                                <th>Dentist</th>
                                <th>Treatment Notes</th>
                                <th>Action</th>
                                <th>Review</th> <!-- New column for Review Link -->
                            </tr>
                        </thead>
                        <tbody>
                            <?php while ($row = $no_show_appointments_result->fetch_assoc()): ?>
                                <tr>
                                    <td class="hidden-column"><?php echo htmlspecialchars($row['appointment_id']); ?></td> <!-- Hidden column -->
                                    <td><?php echo htmlspecialchars($row['appointment_date']); ?></td>
                                    <td><?php echo htmlspecialchars($row['appointment_time']); ?></td>
                                    <td><?php echo htmlspecialchars($row['dentist_first_name'] . ' ' . $row['dentist_last_name']); ?></td>
                                    <td class="treatment-notes">
                                        <?php echo nl2br(htmlspecialchars($row['treatment_notes'] ?: 'N/A')); ?>
                                    </td>
                                    <td>
                                        <?php if ($row['notify_patient'] == 1): ?>
                                            Book for Future Appointment
                                        <?php else: ?>
                                            N/A
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if ($row['review_rating']): ?>
                                            <!-- Display the rating if the user has already submitted a review -->
                                            <span class="rating"><?php echo htmlspecialchars($row['review_rating']); ?> ★</span>
                                        <?php else: ?>
                                            <!-- Review Link -->
                                            <a href="patient_reviews.php?appointment_id=<?php echo htmlspecialchars($row['appointment_id']); ?>">Leave a Review</a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
